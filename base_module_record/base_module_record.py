...
class xElement(minidom.Element):
    ...
    def writexml(self, writer, indent="", addindent="", newl=""):
        writer.write(indent)
        minidom.Element.writexml(self, writer, indent='',
                                 addindent='', newl='')
        writer.write(newl)
...
#            del data[f]
        for key in data.keys():
            ...
            if mod_fields[key]['type'] == 'many2one':
                if isinstance(data[key], bool) or isinstance(data[key], int):
                    result[key] = data[key]
                elif not data[key]:
                    result[key] = False
                else:
                ...
                # continue
                # due to this start stop recording will not record
                # one2many field
    ...
    def generate_yaml(self):
        ...
        if len(recording_data):
            ...
            for rec in recording_data:
                ...
                if self.mode == "workflow":
                    ...
                    yaml_file += "!comment Performing a workflow action %s\
                        on module %s" % (record['action'], record['model']) +\
                        '''\n'''
        ...
        for line in yaml_file.split('\n'):
            line = line.replace("''", "'")
            if (line.find('!record') == 0) or (line.find('!workflow') == 0) or\
                    (line.find('!python') == 0):
                line = "- \n" + "  " + line
