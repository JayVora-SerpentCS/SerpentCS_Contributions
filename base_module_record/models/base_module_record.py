    ...
    def _create_record(self, doc, model, data, record_id, noupdate=False):
        ...
        for key, val in data.items():
            if key in model_pool._columns.keys() and \
                    isinstance(model_pool._columns[key], function_field) and \
                    not model_pool._columns[key].store:
                continue
    ...
    def _create_yaml_record(self, model, data, record_id):
        ...
        for key, val in data.items():
            if ((key in defaults[model]) and
                    (val == defaults[model][key])) and \
                    not(fields[key].get('required', False)):
                continue
    ...
    def get_copy_data(self, model, id, result):
        ...
        for key in data.keys():
            ...
            if mod_fields[key]['type'] == 'many2one':
                #  if type(data[key]) == type(True) or \
                #        type(data[key]) == type(1):
