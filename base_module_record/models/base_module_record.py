...
#        context = dict(context)
        ...
        for key, val in data.items():
            if key in model_pool._columns.keys() and \
                isinstance(model_pool._columns[key], function_field) and not \
                    model_pool._columns[key].store:
                continue
                ...
                for valitem in (val or []):
                    if valitem[0] in (0, 1):
                        ...
                        if valitem[0] == 0:
                            ...
                            newid, update = \
                                self._get_id(fields[key]['relation'],
                                             valitem[1])
                            if not newid:
                                newid = \
                                    self._create_id(fields[key]['relation'],
                                                    valitem[2])
                                valitem[1] = newid
                ...
                for valitem in (val or []):
                    if valitem[0] in (0, 1):
                        if key in model_pool._columns:
                            ...
                            fname = model_pool._inherit_fields[key][2].\
                                _fields_id
                        del valitem[2][fname]  # delete parent_field \
#                                                from child's fields list
                        childrecord = \
                            self._create_yaml_record(fields[key]['relation'],
                                                     valitem[2], None)
...
#        for f in filter(lambda a: isinstance(obj._columns[a],
#                                             fields.function)\
...
#            del data[f]
        ...
        for key in data.keys():
            ...
            if mod_fields[key]['type'] == 'many2one':
                if isinstance(data[key], type(True)) or isinstance(data[key],
                                                                   type(1)):
                    result[key] = data[key]
                elif not data[key]:
                    result[key] = False
                ...
                # continue # due to this start stop recording will not record \
                # one2many field
                ...
                if len(data[key]):
                    ...
                    for rel_id in data[key]:
                        res = [0, 0]
                        if rel == model:  # Avoid to recursion calling of \
                                        # get_copy_data method
                            continue
...
#            del result[v]
    ...
    def generate_xml(self):
        ...
        if len(recording_data):
            # upadate code here to pass blank dict into self replace with \
            # self.id = {} to self.blank_dict = {}
            self.blank_dict = {}
            ...
            for rec in recording_data:
                ...
                if rec[0] == 'query':
                    res_list, noupdate = self._generate_object_xml(rec[1],
                                                                   rec[2],
                                                                   doc,
                                                                   rec[3])
                    data = doc.createElement("data")
        ...
        if len(recording_data):
            ...
            for rec in recording_data:
                ...
                if self.mode == "workflow":
                    ...
                    yaml_file += "!comment Performing a workflow action %s on \
                        module %s" % (record['action'],
                                      record['model']) + '''\n'''
                    ...
                    yaml_file += "!comment Performing an osv_memory action %s \
                        on module %s" % (osv_action['action'],
                                         osv_action['model']) + '''\n'''
