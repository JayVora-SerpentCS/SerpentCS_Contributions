...
class Applicant(models.Model):
    ...
    medical_ids = fields.One2many('hr.applicant.medical.details', 'applicant_id', 'Medical Ref.')
    no_of_medical = fields.Integer('No of Medical Detials', compute='_compute_no_of_medical', readonly=True)
    no_of_medical1 = fields.Integer('No of Medical Detials', compute='_compute_no_of_medical', readonly=True)
    prev_occu_ids = fields.One2many('applicant.previous.occupation', 'applicant_id', 'Prev. Occupation Ref.')
    no_of_prev_occu = fields.Integer('No of Prev. Occupation', compute='_compute_no_of_prev_occu', readonly=True)
    relative_ids = fields.One2many('applicant.relative', 'applicant_id', 'Relative Ref.')
    no_of_relative = fields.Integer('No of Relative', compute='_compute_no_of_relative', readonly=True)
    education_ids = fields.One2many('applicant.education', 'applicant_id', 'Education Ref.')
    no_of_education = fields.Integer('No of Education', compute='_compute_no_of_education', readonly=True)
    prev_travel_ids = fields.One2many('applicant.previous.travel', 'applicant_id', 'Previous Travel Ref.')
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        res = super(Applicant, self).fields_view_get(view_id=view_id, view_type=view_type, toolbar=toolbar, submenu=submenu)
        reports = ir_actions_report.search([('report_name', '=', 'hr_recruitment_extended.applicant_profile')])
    ...
    def create_employee_from_applicant(self):
        ...
        if res.get('res_id', False):
            for applicant in self:
                for medical_detail in self.env['hr.applicant.medical.details'].search([('applicant_id', '=', applicant.id)]):
                    medical_id = self.env['hr.employee.medical.details'].create({'medical_examination': medical_detail.medical_examination,
                    ...
                    medical_attachments = self.env['ir.attachment'].search([('res_model', '=', 'hr.applicant.medical.details'),
                                                                            ('res_id', '=', medical_detail.id)])
                    for medical_attachment in medical_attachments:
                        emp_medical_attachment = medical_attachment.copy()
                        emp_medical_attachment.write({'res_model': 'hr.employee.medical.details', 'res_id': medical_id.id})
                for prev_occupation in self.env['applicant.previous.occupation'].search([('applicant_id', '=', applicant.id)]):
                    occupation_id = self.env['employee.previous.occupation'].create({'from_date': prev_occupation.from_date,
                    ...
                    occupation_attachments = self.env['ir.attachment'].search([('res_model', '=', 'applicant.previous.occupation'),
                    ...
                    for occupation_attachment in occupation_attachments:
                        ...
                        emp_occupation_attachment.write({'res_model': 'employee.previous.occupation', 'res_id': occupation_id.id})
                for relative in self.env['applicant.relative'].search([('applicant_id', '=', applicant.id)]):
                    relative_id = self.env['employee.relative'].create({'relative_type': relative.relative_type,
                                                                        'name': relative.name,
                    ...
                    relative_attachments = self.env['ir.attachment'].search([('res_model', '=', 'applicant.relative'),
                                                                             ('res_id', '=', relative.id)])
                    for relative_attachment in relative_attachments:
                        emp_relative_attachment = relative_attachment.copy()
                        emp_relative_attachment.write({'res_model': 'employee.relative', 'res_id': relative_id.id})
                for education in self.env['applicant.education'].search([('applicant_id', '=', applicant.id)]):
                    education_id = self.env['employee.education'].create({'from_date': education.from_date,
                    ...
                    education_attachments = self.env['ir.attachment'].search([('res_model', '=', 'applicant.education'),
                                                                              ('res_id', '=', education.id)])
                    for education_attachment in education_attachments:
                        emp_education_attachment = education_attachment.copy()
                        emp_education_attachment.write({'res_model': 'employee.education', 'res_id': education_id.id})
                for prev_travel in self.env['applicant.previous.travel'].search([('applicant_id', '=', applicant.id)]):
                    prev_travel_id = self.env['employee.previous.travel'].create({'from_date': prev_travel.from_date,
                                                                                'to_date': prev_travel.to_date,
                                                                                'location': prev_travel.location,
                                                                                'reason': prev_travel.reason,
                                                                                'employee_id': res.get('res_id', False)
                                                                                })
                    prev_travel_attachments = self.env['ir.attachment'].search([('res_model', '=', 'applicant.previous.travel'),
                    ...
                    for prev_travel_attachment in prev_travel_attachments:
                        ...
                        emp_prev_travel_attachment.write({'res_model': 'employee.previous.travel', 'res_id': prev_travel_id.id})
                for language in self.env['applicant.language'].search([('applicant_id', '=', applicant.id)]):
                    language_id = self.env['employee.language'].create({'language': language.language,
                    ...
                    language_attachments = self.env['ir.attachment'].search([('res_model', '=', 'applicant.language'),
                                                                             ('res_id', '=', language.id)])
                    for language_attachment in language_attachments:
                        emp_language_attachment = language_attachment.copy()
                        emp_language_attachment.write({'res_model': 'employee.language', 'res_id': language_id.id})
        return res
...
class ApplicantMedicalDetails(models.Model):
    ...
    urine_drug_serene = fields.Selection([('Negative', 'Negative'), ('Positive', 'Positive')], 'Urine Drug Serene')
...
class ApplicantEducation(models.Model):
    ...
    active = fields.Boolean(string='Active', default=True)
    applicant_id = fields.Many2one('hr.applicant', 'Applicant Ref', ondelete='cascade')
    edu_type = fields.Selection([('Local', 'Local'), ('Abroad', 'Abroad')], string='School Location', default="Local")
    ...
    def onchange_date(self):
        if self.to_date and datetime.strptime(self.to_date, DEFAULT_SERVER_DATE_FORMAT) >= datetime.today():
            ...
            return {'warning': warning}
        if self.from_date and self.to_date and self.from_date > self.to_date:
            warning = {'title': _('User Alert !'), 'message': _('To Date must be greater than From Date !')}
            self.to_date = False
            return {'warning': warning}
...
class ApplicantLanguage(models.Model):
    ...
    def _check_mother_tongue(self):
        if self.mother_tongue and self.applicant_id:
            language_rec = self.search([('applicant_id', '=', self.applicant_id.id), ('mother_tongue', '=', True), ('id', '!=', self.id)], limit=1)
            if language_rec:
