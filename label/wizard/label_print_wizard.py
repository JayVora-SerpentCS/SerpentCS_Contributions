...
class label_print_wizard(models.TransientModel):
    ...
    def default_get(self, cr, uid, fields, context=None):
        if context is None:
            context = {}
        result = super(label_print_wizard, self).default_get(cr, uid, fields,
                                                             context)
        if context.get('label_print'):
            label_print_obj = self.pool.get('label.print')
            label_print_data = label_print_obj.browse(cr, uid, context.get(
                'label_print'), context)
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        context.update({"label_print_id": context.get(
