...
class ResPartner(models.Model):
    ...
    def _compute_available_credit(self):
        ...
        for partner in self:
            ...
            confirm_sale_orders = sale_order_obj.search([
                ('partner_id', '=', partner.id),
                ('state', '=', 'sale'),
    ...
    def check_negative_value_in_credit_limit(self):
        for record in self:
            if record.credit_limit < 0:
                raise ValidationError(
                    self.env._("Credit limit must be a positive value.")
                )
    ...
    def check_nagative_value_in_account_default_credit_limit(self):
        for record in self:
            if record.account_default_credit_limit < 0:
                raise ValidationError(self.env._("Account Default Credit Limit Should Be Positive Value."))