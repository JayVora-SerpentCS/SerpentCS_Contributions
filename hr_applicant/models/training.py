...
class CoursesType(models.Model):
    ...
    _code_unique = models.Constraint(
        'unique (name)',
        'The code of must be unique per course !',
    )
...
class Trainingcourses(models.Model):
    ...
    duration = fields.Integer(string="Course Duration", required=True, default="1")
    duration_type = fields.Selection(
        [("day", "Days"), ("week", "Weeks"), ("month", "Months")], required=True , string="Course Duration"
    )
    local_short_description = fields.Text(string="Course Description")
...
class TrainingClass(models.Model):
    ...
    department = fields.Char(related="course_id.department", readonly=True,string="Department")
    ...
    training_location = fields.Char(
        related="course_id.training_location", readonly=True, string="Training Location"
    )
    training_attendees = fields.Integer(required=True, string="Training Attendees")
    ...
    def write(self, vals):
        res = super(TrainingClass, self).write(vals)
...
class ListOfAttendees(models.Model):
    ...
    _employee_class_unique = models.Constraint(
