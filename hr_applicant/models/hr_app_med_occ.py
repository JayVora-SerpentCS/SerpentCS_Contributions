...
class ApplicantMedicalDetails(models.Model):
    ...
    medical_examination = fields.Char(string="Medical Examination")
    vital_sign = fields.Char(string="Vital sign")
    date = fields.Date(default=fields.Date.context_today, readonly=True, string="Date")
    doc_comment = fields.Char(string="Doctor’s Comments")
...
class ApplicantPreviousOccupation(models.Model):
    ...
    active = fields.Boolean(default=True)
    applicant_id = fields.Many2one("hr.applicant", "Applicant Ref", ondelete="cascade")
    email = fields.Char(string="Reference Email")
