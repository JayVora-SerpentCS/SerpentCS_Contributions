...
class website_ipushp(http.Controller):
    ...
    def ipushp(self, **kwargs):
        return request.render('ipushp.iPushp', {
            'category_data': request.env
                        ['business.category'].sudo().search([]),
            'relation_data': request.env
                        ['relation.relation'].sudo().search([]),
        })

    @http.route(['/page/search'], type='http', auth="public", website=True)
    def search(self, **kwargs):
        return request.render('website.search', {
            'category_data': request.env
                        ['business.category'].sudo().search([]),
        })
    ...
    def find_contacts(self, **kwargs):
        ...
        contact_ids = request.env['business.line'].sudo().search([('category_id', '=', int(category_id))])
        return request.render('website.find_contacts', {
            'category_data': request.env
                    ['business.category'].sudo().search([]),
    ...
    def contact_ipushp(self, **kwargs):
        ...
        if kwargs.get('user_id'):
            employee = hr_emp_obj.sudo().search([('user_id', '=', int(kwargs.get('user_id')))])
            contact_details = {
