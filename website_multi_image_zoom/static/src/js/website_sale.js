odoo.define('website_multi_image_zoom.website_sale', function (require) {
    ...
    $(document).ready(function() {
        $('.oe_website_sale').each(function() {
            ...
            + $('#ex1').find('img').attr("src") + '" src="' +
            $('#ex1').find('img').attr("src") + '"/></div>';
            $('#ex1 span').html(demo);
            ...
            $(oe_website_sale).on('change', 'input.js_variant_change,\
                select.js_variant_change, ul[data-attribute_value_ids]',
                function(ev) {
                var $ul = $(ev.target).closest('ul.js_add_cart_variants');
                var $parent = $ul.closest('.js_product');
                var product_id = $parent.find('input.product_id').first().val();
                var $img = $(this).closest('tr.js_product,\
                    .oe_website_sale').find('span[data-oe-model^="product."]\
                    [data-oe-type="image"] img:first, img.product_detail_img');
                if ($('#variant_img_display').val()) {
                    if (product_id) {
                        ajax.jsonRpc("/get_variant_images", 'call', {
                            'product_id': product_id
                        }).then(function(data) {
                            var str = '<li><img onclick="pro_img_click(this)"\
                             class="xzoom-gallery" width="80"\
                             src="/web/image/product.product/' + product_id +
                             '/image" xpreview="/web/image/product.product/' +
                             product_id + '/image"/></li>';
                            var pro_imgs = data.product_rec;
                            for (var i = 0; i < pro_imgs.length; i++) {
                                str += '<li><img onclick="pro_img_click(this)"\
                                class="xzoom-gallery" width="80"\
                                src="/web/image/product.image/' + pro_imgs[i] +\
                                '/image" xpreview="/web/image/product.image/' +
                                pro_imgs[i] + '/image" /></li>';
                            }
                            var first_append = "<div id='carousel-custom'\
                    ...
                    $img.attr("src", "/website/image/product.template/" +
                        $('#product_img_display').val() + "/image");
                }
            });

            $('ul.js_add_cart_variants', oe_website_sale).each(function() {
                $('input.js_variant_change, select.js_variant_change',
