    ...
    $('.oe_website_sale').each(function() {
        ...
        $(oe_website_sale).on('change', 'input.js_variant_change, select.js_variant_change, ul[data-attribute_value_ids]', function(ev) {
            ...
            if ($('#variant_img_display').val()) {
                if (product_id) {
                    ajax.jsonRpc("/get_variant_images", 'call', {'product_id': product_id}).then(function(data) {
                        var str = '<li><img onclick="pro_img_click(this)" class="image_thumb img-responsive" src="/web/image/product.product/' + product_id + '/image"/></li>';
                        var pro_imgs = data['product_rec'];
                        for (i = 0; i < pro_imgs.length; i++) {
                            str += '<li><img onclick="pro_img_click(this)" class="image_thumb img-responsive" src="/web/image/product.image/' + pro_imgs[i] + '/image"/></li>';
                        }
